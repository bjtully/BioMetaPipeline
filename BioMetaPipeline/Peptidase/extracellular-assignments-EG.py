...
signalp = pd.read_csv(sys.argv[2],sep="\t",index_col=False,comment="#",names=["name","Cmax","pos1","Ymax","pos2","Smax","pos3","Smean","D","Answer","Dmaxcut","Networks-used"])
...
#READ IN MEROPS DATA
MEROPS = pd.read_csv(sys.argv[3],sep="\t",index_col=False,comment='#',names=["target_name","accession_target","query_name","accession_query","E-value_FullSeq","score_FullSeq","bias_FullSeq","E-value_Best1Domain","score_Best1Domain","bias_Best1Domain","exp","reg","clu","ov","env","dom","rep","inc","gene","description"])
...
FinalCountTablePFAM = pd.DataFrame.from_dict(GenomeDict,orient='index').stack().str.get_dummies().sum(level=0).sort_index()
